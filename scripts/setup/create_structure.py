...
def check_arguments():
    ...
    if not args.domain_root.endswith("domains"):
        print(
            f'The specified domain root does not appear to be a valid domain root (should end with "domains"): {args.domain_root}'
        )
        exit(1)
...
async def create_solution(output_path: str, solution_name: str) -> int | None:
    ...
    print(
        ...
        stderr.decode()
        if stderr
        else f"Solution {solution_name}.sln created successfully.",
    )
    return process.returncode


async def print_structure(domain_path: str) -> int | None:
    # Create the subprocess
    process = await asyncio.create_subprocess_exec(
        "ls",
        "-lR",
...
def create_settings_files(domain_path: str)
    files_to_create = [".env.local",  ".secrets.local", ".env.example", "secrets.example"]
    settings_dir = "settings"
    subdir_path = os.path.join(domain_path, settings_dir)
    if not os.path.exists(subdir_path)
        os.makedirs(subdir_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The domain root directory where the structure should be created.",
    )
    parser.add_argument(
        "-l",
        "--list",
        required=True,
        help="List of domains to create, ie: [customer,order].",
        nargs="*",
    )
    parser.add_argument(
        "-a",
        "--additional_subdirs",
        nargs="*",
        help="Additional subdirectories to create within each domain.",
    )
    ...
    try:
        ...
        for domain in args.list:
            ...
            create_settings_files(domain_path)
